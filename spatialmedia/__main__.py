...
def main(main_args):
    ...
    parser.add_argument(
        ...
        "--inject",
        action="store_true",
        help="injects spatial media metadata into the first file specified (.mp4 or "
        ".mov) and saves the result to the second file specified",
    )
    parser.add_argument(
        "-2", "--v2", action="store_true", help="Uses v2 of the video metadata spec"
    )
    ...
    video_group.add_argument(
        ...
        action="store",
        default=None,
        help='crop region. Must specify 6 integers in the form of "w:h:f_w:f_h:x:y"'
    ...
    video_group.add_argument(
        ...
        action="store",
        default=None,
        help="Equirect projection bounds for VR180. Must specify 4 integers "
    ...
    if args.inject:
        ...
        if args.spatial_audio:
            ...
            if not metadata.audio:
                ...
                if spatial_audio_description.is_supported:
                    ...
                    console(
                        "Audio has %d channel(s) and is not a supported "
        ...
        if metadata.video or metadata.projection or metadata.stereo_mode:
            metadata_utils.inject_metadata(
                args.file[0], args.file[1], metadata, console
            )
        else:
